...
export type TestCollectionCreate = Omit<
  TestCollection,
...
export type TestCollectionPick<K extends keyof TestCollection> = Pick<
  TestCollection,
  K
>;
export type TestCollectionOmit<K extends keyof TestCollection> = Omit<
  TestCollection,
  K
>;
...
export type TestCollectionQuery = {
  [K in keyof TestCollection]?:
    | TestCollection[K]
    | { $in?: TestCollection[K][] }
    | { $ne?: TestCollection[K] }
    | { $exists?: boolean }
    | { $gt?: TestCollection[K] }
    | { $gte?: TestCollection[K] }
    | { $lt?: TestCollection[K] }
    | { $lte?: TestCollection[K] }
    | { $regex?: string }
    | { $contains?: string };
} & {
...
export type TestCollectionInput = Omit<
  TestCollection,
  | "$id"
  | "$createdAt"
  | "$updatedAt"
