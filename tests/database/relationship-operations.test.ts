...
import { DatabaseException, NotFoundException, DuplicateException, RelationshipException } from '@errors/index.js';
...
describe('Relationship Operations', () => {
    ...
    describe('createRelationship', () => {
        describe('OneToOne relationships', () => {
            test('should create one-to-one relationship', async () => {
                ...
                const userAttributes = usersCollection.get('attributes') as Doc<Attribute>[];
                const profileAttr = userAttributes.find(attr => attr.get('$id') === 'profile');
                ...
                const postAttributes = postsCollection.get('attributes') as Doc<Attribute>[];
                const userAttr = postAttributes.find(attr => attr.get('$id') === 'user');
            ...
            test('should create one-way one-to-one relationship', async () => {
                ...
                const usersCollection = await db.getCollection(usersCollectionId);
                const userAttributes = usersCollection.get('attributes') as Doc<Attribute>[];
                const featuredPostAttr = userAttributes.find(attr => attr.get('$id') === 'featured_post');
                ...
                const postsCollection = await db.getCollection(postsCollectionId);
                const postAttributes = postsCollection.get('attributes') as Doc<Attribute>[];
                const featuredUserAttr = postAttributes.find(attr => attr.get('$id') === 'featured_user');
        ...
        describe('OneToMany relationships', () => {
            test('should create one-to-many relationship', async () => {
                ...
                const usersCollection = await db.getCollection(usersCollectionId);
                const userAttributes = usersCollection.get('attributes') as Doc<Attribute>[];
                const postsAttr = userAttributes.find(attr => attr.get('$id') === 'posts');

                expect(postsAttr).toBeDefined();
                expect(postsAttr?.get('options').relationType).toBe(RelationEnum.OneToMany);

                // Check child collection (posts)
                const postsCollection = await db.getCollection(postsCollectionId);
                const postAttributes = postsCollection.get('attributes') as Doc<Attribute>[];
                const authorAttr = postAttributes.find(attr => attr.get('$id') === 'author');

                expect(authorAttr).toBeDefined();
                expect(authorAttr?.get('options').relationType).toBe(RelationEnum.OneToMany);
            });
        });
        ...
        describe('ManyToOne relationships', () => {
            test('should create many-to-one relationship', async () => {
                ...
                expect(authorAttr).toBeDefined();
                expect(authorAttr?.get('options').relationType).toBe(RelationEnum.ManyToOne);
                ...
                expect(postsAttr).toBeDefined();
                expect(postsAttr?.get('options').relationType).toBe(RelationEnum.ManyToOne);
            });
        });
        ...
        describe('ManyToMany relationships', () => {
            test('should create many-to-many relationship', async () => {
                ...
                const postsCollection = await db.getCollection(postsCollectionId);
                const postAttributes = postsCollection.get('attributes') as Doc<Attribute>[];
                const tagsAttr = postAttributes.find(attr => attr.get('$id') === 'tags');

                expect(tagsAttr).toBeDefined();
                expect(tagsAttr?.get('options').relationType).toBe(RelationEnum.ManyToMany);

                const tagsCollection = await db.getCollection(tagsCollectionId);
                const tagAttributes = tagsCollection.get('attributes') as Doc<Attribute>[];
                const postsAttr = tagAttributes.find(attr => attr.get('$id') === 'posts');

                expect(postsAttr).toBeDefined();
                expect(postsAttr?.get('options').relationType).toBe(RelationEnum.ManyToMany);
            });
        });

        describe('relationship validation', () => {
            test('should throw error for non-existent related collection', async () => {
                await expect(db.createRelationship({
                    collectionId: usersCollectionId,
            ...
            test('should use default relationship names', async () => {
                ...
                const usersCollection = await db.getCollection(usersCollectionId);
                const userAttributes = usersCollection.get('attributes') as Doc<Attribute>[];
                const relationAttr = userAttributes.find(attr =>
                    attr.get('$id') === postsCollectionId
                );
    ...
    describe('updateRelationship', () => {
        ...
        test('should update relationship key names', async () => {
            ...
            const usersCollection = await db.getCollection(usersCollectionId);
            const userAttributes = usersCollection.get('attributes') as Doc<Attribute>[];
            const postsAttr = userAttributes.find(attr => attr.get('$id') === 'user_posts');
            ...
            const postsCollection = await db.getCollection(postsCollectionId);
            const postAttributes = postsCollection.get('attributes') as Doc<Attribute>[];
            const authorAttr = postAttributes.find(attr => attr.get('$id') === 'post_author');
        ...
        test('should throw error for ManyToMany relationships', async () => {
            ...
            await expect(db.updateRelationship({
                collectionId: postsCollectionId,
                id: 'tags',
                newKey: 'post_tags'
    ...
    describe('deleteRelationship', () => {
        ...
        test('should throw error for non-existent relationship', async () => {
            await expect(db.deleteRelationship(usersCollectionId, 'nonexistent'))
                .rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('relationship data operations', () => {
        ...
        test('should create documents with relationship data', async () => {
            // Create a user
            const user = await db.createDocument(usersCollectionId, new Doc({
                name: 'John Doe',
                email: 'john@example.com'
            }));

            // Create a post with author relationship
            const post = await db.createDocument(postsCollectionId, new Doc({
                title: 'My First Post',
                content: 'Hello World!',
                author: user.getId()
            }));
        ...
        test('should handle relationship queries with populate', async () => {
            ...
            // Query with populate
            const populatedPost = await db.getDocument(postsCollectionId, post.getId(), [
        ...
        test('should handle cascade deletion', async () => {
            ...
            const post1 = await db.createDocument(postsCollectionId, new Doc({
                title: 'Post 1',
                author: user.getId()
            }));

            const post2 = await db.createDocument(postsCollectionId, new Doc({
                title: 'Post 2',
                author: user.getId()
            }));
            ...
            const remainingPost1 = await db.getDocument(postsCollectionId, post1.getId());
            const remainingPost2 = await db.getDocument(postsCollectionId, post2.getId());
        ...
        test('should handle restrict deletion', async () => {
            ...
            await db.createDocument(postsCollectionId, new Doc({
                title: 'Protected Post',
                author: user.getId()
            }));

            // Should not be able to delete user with related posts
            await expect(db.deleteDocument(usersCollectionId, user.getId()))
        ...
        test('should handle set null deletion', async () => {
            ...
            const orphanedPost = await db.getDocument(postsCollectionId, post.getId());
    ...
    describe('edge cases', () => {
        ...
        test('should handle multiple relationships between same collections', async () => {
            ...
            const authoredAttr = userAttributes.find(attr => attr.get('$id') === 'authored_posts');
            const reviewedAttr = userAttributes.find(attr => attr.get('$id') === 'reviewed_posts');
