...
import { NotFoundException, DatabaseException, StructureException, AuthorizationException } from '@errors/index.js';
...
describe('Document Operations', () => {
    ...
    describe('createDocument', () => {
        test('should create a basic document', async () => {
            ...
            const document = await db.createDocument(testCollectionId, new Doc(documentData));
        ...
        test('should throw error for missing required fields', async () => {
            ...
            await expect(db.createDocument(testCollectionId, new Doc(documentData)))
    ...
    describe('createDocuments', () => {
        test('should create multiple documents', async () => {
            ...
            const documents = await db.createDocuments(testCollectionId, documentsData);
        ...
        test('should auto-generate IDs for all documents', async () => {
            const documentsData = Array.from({ length: 5 }, (_, i) =>
                new Doc({ name: `Batch User ${i}` })
            );
        ...
        test('should validate all documents before creation', async () => {
            ...
            await expect(db.createDocuments(testCollectionId, documentsData))
                .rejects.toThrow(StructureException);
        });
    });
    ...
    describe('getDocument', () => {
        test('should get existing document', async () => {
            ...
            const created = await db.createDocument(testCollectionId, new Doc(documentData));
        ...
        test('should get document with selected fields', async () => {
            const documentData = { name: 'Select Test', age: 25, email: 'select@test.com' };
            const created = await db.createDocument(testCollectionId, new Doc(documentData));

            const retrieved = await db.getDocument(testCollectionId, created.getId(), qb =>
                qb.select('name', 'age')
            );
    ...
    describe('updateDocument', () => {
        ...
        test('should update only specified fields', async () => {
            const documentData = { name: 'Partial Update', age: 25, email: 'original@test.com' };
            const created = await db.createDocument(testCollectionId, new Doc(documentData));
        ...
        test('should throw error for non-existent document', async () => {
            ...
            const updated = await db.updateDocument(testCollectionId, nonExistentId, updates);
        ...
        test('should validate structure during update', async () => {
            ...
            await expect(db.updateDocument(testCollectionId, created.getId(), updates))
                .rejects.toThrow();
        });
    });
    ...
    describe('updateDocuments', () => {
        ...
        test('should respect batch size', async () => {
            ...
            const documentsData = Array.from({ length: 10 }, (_, i) =>
                new Doc({ name: `Batch User ${i}`, age: i })
            );
            await db.createDocuments(testCollectionId, documentsData);
    ...
    describe('deleteDocument', () => {
        test('should delete existing document', async () => {
            ...
            const deleted = await db.deleteDocument(testCollectionId, created.getId());
    ...
    describe('deleteDocuments', () => {
        ...
        test('should delete with query filters', async () => {
            ...
            const deletedIds = await db.deleteDocuments(
                testCollectionId,
