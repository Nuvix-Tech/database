...
import { DatabaseException, NotFoundException, DuplicateException } from '@errors/index.js';
...
describe('Attribute Operations', () => {
    ...
    describe('createAttribute', () => {
        ...
        test('should create float attribute', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const attributes = collection.get('attributes') as Doc<Attribute>[];
            const ratingAttr = attributes.find(attr => attr.get('$id') === 'rating');
        ...
        test('should create boolean attribute', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const attributes = collection.get('attributes') as Doc<Attribute>[];
            const activeAttr = attributes.find(attr => attr.get('$id') === 'active');
        ...
        test('should create JSON attribute', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const attributes = collection.get('attributes') as Doc<Attribute>[];
            const metadataAttr = attributes.find(attr => attr.get('$id') === 'metadata');
        ...
        test('should create attribute with filters', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const attributes = collection.get('attributes') as Doc<Attribute>[];
            const descAttr = attributes.find(attr => attr.get('$id') === 'description');
        ...
        test('should throw error for relationship attribute type', async () => {
            ...
            await expect(db.createAttribute(testCollectionId, attribute))
    ...
    describe('createAttributes', () => {
        test('should create multiple attributes', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const collectionAttributes = collection.get('attributes') as Doc<Attribute>[];

            expect(collectionAttributes).toHaveLength(5); // 2 initial + 3 new

            const emailAttr = collectionAttributes.find(attr => attr.get('$id') === 'email');
            const phoneAttr = collectionAttributes.find(attr => attr.get('$id') === 'phone');
            const activeAttr = collectionAttributes.find(attr => attr.get('$id') === 'active');
        ...
        test('should throw error for empty attributes array', async () => {
            await expect(db.createAttributes(testCollectionId, []))
                .rejects.toThrow(DatabaseException);
        });
    });
    ...
    describe('updateAttributeRequired', () => {
        test('should update required status to true', async () => {
            ...
            const updated = await db.updateAttributeRequired(testCollectionId, 'email', true);

            expect(updated.get('required')).toBe(true);
        });

        test('should update required status to false', async () => {
            const updated = await db.updateAttributeRequired(testCollectionId, 'name', false);

            expect(updated.get('required')).toBe(false);
        });

        test('should throw error for non-existent attribute', async () => {
            await expect(db.updateAttributeRequired(testCollectionId, 'nonexistent', true))
    ...
    describe('updateAttributeFormat', () => {
        test('should update string attribute format', async () => {
            ...
            const updated = await db.updateAttributeFormat(testCollectionId, 'email', 'email');
        ...
        test('should throw error for invalid format', async () => {
            ...
            await expect(db.updateAttributeFormat(testCollectionId, 'email', 'invalid_format'))
                .rejects.toThrow(DatabaseException);
        });
    });
    ...
    describe('updateAttributeFormatOptions', () => {
        test('should update format options', async () => {
            ...
            const updated = await db.updateAttributeFormatOptions(testCollectionId, 'description', formatOptions);
    ...
    describe('updateAttributeFilters', () => {
        test('should update filters', async () => {
            ...
            const updated = await db.updateAttributeFilters(testCollectionId, 'secret', filters);
    ...
    describe('updateAttributeDefault', () => {
        test('should update default value', async () => {
            const updated = await db.updateAttributeDefault(testCollectionId, 'age', 25);

            expect(updated.get('default')).toBe(25);
        });

        test('should set default to null', async () => {
            const updated = await db.updateAttributeDefault(testCollectionId, 'age', null);

            expect(updated.get('default')).toBeNull();
        });

        test('should throw error for required attribute', async () => {
            await expect(db.updateAttributeDefault(testCollectionId, 'name', 'default_name'))
                .rejects.toThrow(DatabaseException);
        });
    });
    ...
    describe('updateAttribute', () => {
        test('should update attribute type and size', async () => {
            ...
            const updated = await db.updateAttribute(testCollectionId, 'description', {
                size: 1000
            });
        ...
        test('should throw error for invalid size', async () => {
            await expect(db.updateAttribute(testCollectionId, 'name', {
    ...
    describe('deleteAttribute', () => {
        test('should delete existing attribute', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const attributes = collection.get('attributes') as Doc<Attribute>[];
            const tempAttr = attributes.find(attr => attr.get('$id') === 'temp_attr');

            expect(tempAttr).toBeUndefined();
        });

        test('should throw error for non-existent attribute', async () => {
            await expect(db.deleteAttribute(testCollectionId, 'nonexistent'))
                .rejects.toThrow(NotFoundException);
        });

        test('should throw error for metadata collection', async () => {
            await expect(db.deleteAttribute(Database.METADATA, 'any_attr'))
                .rejects.toThrow(DatabaseException);
        });
    ...
    describe('renameAttribute', () => {
        test('should rename existing attribute', async () => {
            ...
            const renamed = await db.renameAttribute(testCollectionId, 'old_email', 'new_email');
            expect(renamed).toBe(true);

            const collection = await db.getCollection(testCollectionId);
            const attributes = collection.get('attributes') as Doc<Attribute>[];

            const oldAttr = attributes.find(attr => attr.get('$id') === 'old_email');
            const newAttr = attributes.find(attr => attr.get('$id') === 'new_email');
        ...
        test('should throw error for non-existent collection', async () => {
            await expect(db.renameAttribute('nonexistent', 'attr', 'new_attr'))
                .rejects.toThrow(NotFoundException);
        });

        test('should throw error for non-existent attribute', async () => {
            await expect(db.renameAttribute(testCollectionId, 'nonexistent', 'new_name'))
                .rejects.toThrow(NotFoundException);
        });

        test('should throw error for duplicate name', async () => {
            await expect(db.renameAttribute(testCollectionId, 'age', 'name'))
                .rejects.toThrow(DuplicateException);
        });
    });
    ...
    describe('edge cases', () => {
        ...
        test('should validate attribute size limits', async () => {
            ...
            await expect(db.createDocument(testCollectionId, new Doc({
                name: 'Test',
