...
describe('Collection Operations', () => {
    ...
    describe('createCollection', () => {
        ...
        test('should create collection with attributes', async () => {
            ...
            const attributesInCollection = collection.get('attributes') as Doc<Attribute>[];
    ...
    describe('getCollection', () => {
        ...
        test('should throw error when throwOnNotFound is true', async () => {
            await expect(db.getCollection('non_existent', true))
    ...
    describe('updateCollection', () => {
        ...
        test('should throw error for non-existent collection', async () => {
            await expect(db.updateCollection({
                id: 'non_existent',
                permissions: [Permission.read(Role.any())],
                documentSecurity: false
            })).rejects.toThrow(NotFoundException);
    ...
    describe('listCollections', () => {
        test('should list collections with default pagination', async () => {
            const collectionIds = Array.from({ length: 3 }, (_, i) => `list_test_${i}_${Date.now()}`);
        ...
        test('should respect offset parameter', async () => {
            ...
            expect(offsetCollections.length).toBeLessThanOrEqual(allCollections.length);
        });
    });
    ...
    describe('deleteCollection', () => {
        ...
        test('should throw error for non-existent collection', async () => {
            await expect(db.deleteCollection('non_existent'))
                .rejects.toThrow(NotFoundException);
        });
        ...
        test('should handle relationships during deletion', async () => {
            ...
            const relationshipAttr = attributes.find(attr => attr.get('$id') === 'parent');
    ...
    describe('getSizeOfCollection', () => {
        ...
        test('should throw error for non-existent collection', async () => {
            await expect(db.getSizeOfCollection('non_existent'))
                .rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('getSizeOfCollectionOnDisk', () => {
        ...
        test('should throw error for non-existent collection', async () => {
            await expect(db.getSizeOfCollectionOnDisk('non_existent'))
                .rejects.toThrow(NotFoundException);
        });
    });
