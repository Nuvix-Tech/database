...
import { DatabaseException, NotFoundException, DuplicateException, LimitException } from '@errors/index.js';
...
describe('Index Operations', () => {
    ...
    describe('createIndex', () => {
        ...
        test('should create unique index', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const indexes = collection.get('indexes') as any[];
            const emailIndex = indexes.find(idx => idx.get('$id') === 'email_unique');
        ...
        test('should create fulltext index', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const indexes = collection.get('indexes') as any[];
            const descIndex = indexes.find(idx => idx.get('$id') === 'description_fulltext');
        ...
        test('should create composite index', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const indexes = collection.get('indexes') as any[];
            const compositeIndex = indexes.find(idx => idx.get('$id') === 'name_age_composite');
        ...
        test('should create index with custom orders', async () => {
            ...
            const collection = await db.getCollection(testCollectionId);
            const indexes = collection.get('indexes') as any[];
            const orderedIndex = indexes.find(idx => idx.get('$id') === 'name_age_ordered');
        ...
        test('should create multiple indexes on same attribute', async () => {
            ...
            expect(indexes.filter(idx => idx.get('attributes').includes('email'))).toHaveLength(2);
        });
    });
    ...
    describe('deleteIndex', () => {
        ...
        test('should handle deletion of non-existent index', async () => {
            const deleted = await db.deleteIndex(testCollectionId, 'nonexistent_index');
        ...
        test('should delete multiple indexes', async () => {
            ...
            await db.createIndex(testCollectionId, 'index3', IndexEnum.Key, ['email']);
            ...
            expect(indexes.filter(idx => ['index1', 'index2', 'index3'].includes(idx.get('$id')))).toHaveLength(0);
        });
    });
    ...
    describe('renameIndex', () => {
        test('should rename existing index', async () => {
            ...
            const renamed = await db.renameIndex(testCollectionId, 'old_index_name', 'new_index_name');
            expect(renamed).toBe(true);

            const collection = await db.getCollection(testCollectionId);
            const indexes = collection.get('indexes') as any[];

            const oldIndex = indexes.find(idx => idx.get('$id') === 'old_index_name');
            const newIndex = indexes.find(idx => idx.get('$id') === 'new_index_name');
        ...
        test('should throw error for non-existent collection', async () => {
            await expect(db.renameIndex('nonexistent', 'old_name', 'new_name'))
                .rejects.toThrow(NotFoundException);
        });

        test('should throw error for non-existent index', async () => {
            await expect(db.renameIndex(testCollectionId, 'nonexistent_index', 'new_name'))
                .rejects.toThrow(NotFoundException);
        });
        ...
        test('should throw error for duplicate new name', async () => {
            ...
            await expect(db.renameIndex(testCollectionId, 'index2', 'index1'))
                .rejects.toThrow(DuplicateException);
        });
        ...
        test('should preserve index properties during rename', async () => {
            ...
            await db.renameIndex(testCollectionId, 'complex_index', 'renamed_complex');

            const collection = await db.getCollection(testCollectionId);
            const indexes = collection.get('indexes') as any[];
            const renamedIndex = indexes.find(idx => idx.get('$id') === 'renamed_complex');
