...
export class Permissions extends Roles implements Validator {
  ...
  constructor(
    ...
    if (maxLength < 0) {
      throw new Error(
        "Maximum length of permissions must be a non-negative number.",
      );
    }
    ...
    if (
      ...
      allowedPermissions.some(
        (perm) => !Object.values(PermissionEnum).includes(perm),
      )
    ) {
      throw new Error(
        "Allowed permissions must be an array containing valid PermissionName enum values.",
      );
    }
    this.allowedPermissions = allowedPermissions;
  }
    ...
    for (const permissionString of permissions) {
      if (
        !(
          ...
          permissionString instanceof Permission
        )
      ) {
      ...
      try {
        ...
        this.message =
          error instanceof Error
            ? error.message
            : "Failed to parse permission string.";
        return false;
      }
    }
    return true;
