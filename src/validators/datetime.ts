...
export class Datetime implements Validator {
  private static readonly DEFAULT_MIN_DATE = new Date(
    "0000-01-01T00:00:00.000Z",
  );
  private static readonly DEFAULT_MAX_DATE = new Date(
  ...
  public $valid(value: any): boolean {
    ...
    const now = new Date();
    const requiredFutureDate = new Date(
      now.getTime() + this.offsetSeconds * 1000,
    );
    ...
    if (
      ...
      date.getTime() <= requiredFutureDate.getTime()
    ) {
      return false;
    }

    if (
      date.getTime() < this.min.getTime() ||
