...
export function Column(options?: ColumnOptions): any {
  return function (target: Object, propertyKey: string | symbol) {
    ...
    if ((options?.type === 'datetime' || options?.type === 'timestamp') && !filters.includes('datetime')) {
      filters.push('datetime');
    }

    columns.push(new Document({
      $id: propertyKey,
      key: propertyKey,
      type: options?.type ? typeMap[options?.type] : options?.type,
      size: options?.size || options?.length || undefined,
      required: options?.required !== undefined ? options.required : !options?.nullable,
