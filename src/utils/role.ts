...
export class Role {
    ...
    public toString(): string {
        ...
        if (this._dimension !== null) {
            if (this._identifier === null && this._role !== RoleName.ANY && this._role !== RoleName.GUESTS) {
                str += `/${this._dimension}`;
    ...
    public static parse(roleString: string): Role {
        ...
        if (colonSplit.length === 2) {
            ...
            if (slashSplit.length > 2) {
                throw new StructureException("Invalid role format: too many slashes in identifier/dimension section.");
            }
        ...
        } else { // No colon means no identifier, but could have dimension directly
            const slashSplit = roleNamePart.split("/");
            if (slashSplit.length > 2) {
                throw new StructureException("Invalid role format: too many slashes in roleName section.");
            }
            if (slashSplit.length === 2) {
        ...
        if (dimensionPart !== null && identifierPart === null && colonSplit.length === 2) {
            // Case: "role:/dimension" -> invalid as identifier must precede dimension if colon is used
            throw new StructureException("Invalid role format: dimension cannot follow an empty identifier with a colon.");
        }

        // Additional sanity check for `any` and `guests` roles, which should not have identifier/dimension
        if ((roleNamePart === RoleName.ANY || roleNamePart === RoleName.GUESTS) && (identifierPart !== null || dimensionPart !== null)) {
            throw new StructureException(`Role "${roleNamePart}" cannot have an identifier or dimension.`);
        }
     ...
     * @returns {Role}
     */
    public static user(identifier: string, status: UserDimension | null = null): Role {
        if (!identifier) throw new StructureException("User role must have an identifier.");
     ...
     * @returns {Role}
     */
    public static team(identifier: string, dimension: string | null = null): Role {
        if (!identifier) throw new StructureException("Team role must have an identifier.");
    ...
    public static label(identifier: string): Role {
        if (!identifier) throw new StructureException("Label role must have an identifier.");
    ...
    public static member(identifier: string): Role {
        if (!identifier) throw new StructureException("Member role must have an identifier.");
