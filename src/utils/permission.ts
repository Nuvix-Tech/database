...
export class Permission {
  ...
  public static parse(permissionString: string): Permission {
    ...
    if (!this.isValidPermission(permissionName)) {
      throw new StructureException(
        `Invalid permission type: "${permissionName}".`,
      );
    }
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new StructureException(
          `Failed to parse role from permission string: ${error.message}`,
        );
      }
      throw new StructureException(
        "Failed to parse role from permission string.",
      );
    }
  }
  ...
  public static aggregate(
    ...
    for (const permission of permissions) {
      ...
      if (Object.keys(this.aggregates).includes(permissionName)) {
        ...
        for (const subType of this.aggregates[
          permissionName
        ] as PermissionEnum[]) {
          if (allowed.includes(subType)) {
            const newPermissionString = new Permission(
              subType,
              role,
            ).toString();
