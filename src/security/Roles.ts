...
export class Roles {
  ...
  public static readonly CONFIG: Record<string, { identifier: { allowed: boolean; required: boolean; }; dimension: { allowed: boolean; required: boolean; options?: string[]; }; }> = {
    [Roles.ROLE_ANY]: {
  ...
  public isValid(roles: any): boolean {
    ...
    for (const role of roles) {
      ...
      const isAllowed = this.allowed.some(allowed => role.startsWith(allowed));
      if (!isAllowed) {
  ...
  protected isValidRole(role: string, identifier: string, dimension: string): boolean {
    ...
    if (identifierAllowed && identifier && role === Roles.ROLE_LABEL && !label.isValid(identifier)) {
      this.message = 'Role "' + role + '" identifier value is invalid: ' + label.getDescription();
      return false;
    } else if (identifierAllowed && identifier && role !== Roles.ROLE_LABEL && !key.isValid(identifier)) {
      this.message = 'Role "' + role + '" identifier value is invalid: ' + key.getDescription();
      return false;
    }

    // Process dimension configuration
    const { allowed: dimensionAllowed, required: dimensionRequired, options = [] } = config.dimension;
    ...
    if (dimensionAllowed && dimension && !options.includes(dimension)) {
      this.message = 'Role "' + role + '" dimension value is invalid. Must be one of: ' + options.join(', ') + '.';
      return false;
    }
