...
export default class Permission {
  ...
  private static isValidPermission(permission: string): boolean {
    return this.aggregates.hasOwnProperty(permission) || Database.PERMISSIONS.includes(permission);
  }
  ...
  public static read(role: Role): string {
    const permission = new Permission('read', role.getRole(), role.getIdentifier(), role.getDimension());
    return permission.toString();
  }

  public static create(role: Role): string {
    const permission = new Permission('create', role.getRole(), role.getIdentifier(), role.getDimension());
    return permission.toString();
  }

  public static update(role: Role): string {
    const permission = new Permission('update', role.getRole(), role.getIdentifier(), role.getDimension());
    return permission.toString();
  }

  public static delete(role: Role): string {
    const permission = new Permission('delete', role.getRole(), role.getIdentifier(), role.getDimension());
    return permission.toString();
  }

  public static write(role: Role): string {
    const permission = new Permission('write', role.getRole(), role.getIdentifier(), role.getDimension());
    return permission.toString();
  }
}