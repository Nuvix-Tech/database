...
export class MariaDB extends Sql implements Adapter {
    ...
    async startTransaction(): Promise<boolean> {
        try {
            ...
            throw this.processException(
                e,
                "MariaDB adapter start transaction failed",
            );
        }
    }
    ...
    async commitTransaction() {
        try {
            ...
            throw this.processException(
                e,
                "MariaDB adapter commit transaction failed",
            );
        }
    }
    ...
    async rollbackTransaction() {
        try {
            ...
            throw this.processException(
                e,
                "MariaDB adapter rollback transaction failed",
            );
        }
    }
    ...
    public getSQLCondition(query: Query): string {
        ...
        query.setAttribute(
            attributeMap[query.getAttribute()] || query.getAttribute(),
        );
        ...
        switch (query.getMethod()) {
            ...
            case Query.TYPE_AND:
                const conditions = query
                    .getValue()
                    .map((q: Query) => this.getSQLCondition(q));
                const method = query.getMethod().toUpperCase();
                return conditions.length
                    ? ` ${method} (${conditions.join(" AND ")})`
                    : "";
    ...
    public async createCollection(
        ...
        const attributeSql = attributes
            .map((attribute) => {
                ...
                if (
                    ...
                    const { relationType, twoWay, side } =
        ...
        const indexSql = indexes
            .map((index) => {
                const indexId = this.filter(
                    index.getAttribute("name", index.getAttribute("$id")),
                );
                const indexType = index.getAttribute("type");

                const indexAttributes = index
                    .getAttribute("attributes", [])
                    ?.map((attribute: string, nested: number) => {
                        let indexLength =
                            index.getAttribute("lengths", [])?.[nested] ?? "";
                        indexLength = indexLength ? `(${indexLength})` : "";
                        let indexOrder =
                            index.getAttribute("orders", [])?.[nested] ?? "";
                ...
                if (
                    this.sharedTables &&
                    indexType !== Database.INDEX_FULLTEXT
                ) {
    ...
    public async createDocuments(
        ...
        for (const batch of batches) {
            ...
            batch.forEach((document, index) => {
                ...
                placeholders.push(
                    `(${Object.keys(filteredAttributes)
                        .map(() => "?")
                        .join(", ")})`,
                );
                bindValues.push(
                ...
                Database.PERMISSIONS.forEach((type) => {
                    document
                        ...
                        ?.forEach((permission) => {
                            permissions.push(
                                `('${type}', '${permission.replace(/"/g, "")}', '${document.getId()}'${this.sharedTables ? `, ${this.tenantId}` : ""})`,
                            );
                        });
                });
            });
      ...
      `;
          ...
          `;
                ...
                throw this.processException(
                    e,
                    "Batch Document Creation Failed.",
                );
            }
        }
    ...
    private async getCurrentPermissions(name: string, documentId: string) {
        ...
        const [permissions] = await this.pool.query<any>(sql, [
            documentId,
            this.tenantId,
        ]);
    ...
    public async find(
        ...
        orderAttributes.forEach((attribute, i) => {
            ...
            if (i === 0 && cursor) {
                const cursorValue =
                    cursor[attribute === "_uid" ? "$id" : attribute] || null;
        ...
        if (!hasIdAttribute) {
            orders.push(
                `table_main._id ${cursorDirection === Database.CURSOR_AFTER ? "ASC" : "DESC"}`,
            );
        }
    ...
    `;
        ...
        try {
            ...
            return (
                Number(result[0]["SUM(FS_BLOCK_SIZE + ALLOCATED_SIZE)"]) || 0
            );
        } catch (e) {
            this.logger.error(e);
            throw this.processException(e);
    ...
    protected getAttributeProjection(
        ...
        selections = selections.filter(
            (selection) =>
                !["$id", "$permissions", "$collection"].includes(selection),
        );
