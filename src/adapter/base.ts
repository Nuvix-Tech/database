...
import { Index } from "../core/types/indexes";
...
export interface Adapter {
  ...
  createAttribute(collection: string, id: string, type: string, size: number, signed?: boolean, array?: boolean): Promise<boolean>;

  updateAttribute(collection: string, id: string, type: string, size: number, signed?: boolean, array?: boolean, newKey?: string): Promise<boolean>;

  deleteAttribute(collection: string, id: string): Promise<boolean>;

  renameAttribute(collection: string, oldName: string, newName: string): Promise<boolean>;

  createRelationship(collection: string, relatedCollection: string, type: string, twoWay?: boolean, id?: string, twoWayKey?: string): Promise<boolean>;

  updateRelationship(collection: string, relatedCollection: string, type: string, twoWay: boolean, key: string, twoWayKey: string, side: string, newKey?: string, newTwoWayKey?: string): Promise<boolean>;

  deleteRelationship(collection: string, relatedCollection: string, type: string, twoWay: boolean, key: string, twoWayKey: string, side: string): Promise<boolean>;

  renameIndex(collection: string, oldName: string, newName: string): Promise<boolean>;

  createIndex(collection: string, id: string, type: string, attributes: string[], lengths: number[], orders: string[]): Promise<boolean>;
  ...
  createDocuments(collection: string, documents: Document[], batchSize?: number): Promise<Document[]>;

  updateDocument(collection: string, id: string, document: Document): Promise<Document>;

  updateDocuments(collection: string, updates: Document, documents: Document[]): Promise<number>;

  increaseDocumentAttribute(collection: string, id: string, attribute: string, value: number, updatedAt: string, min?: number, max?: number): Promise<boolean>;

  deleteDocument(collection: string, uid: string): Promise<boolean>;

  deleteDocuments(collection: string, ids: string[]): Promise<number>;

  find(collection: string, queries?: Query[], limit?: number, offset?: number | null, orderAttributes?: string[], orderTypes?: string[], cursor?: any, cursorDirection?: typeof Database.CURSOR_AFTER | typeof Database.CURSOR_BEFORE, forPermission?: string): Promise<Document[]>;

  count(collection: string, queries?: Query[], max?: number | null): Promise<number>;

  sum(collection: string, attribute: string, queries: Query[], max: number | null): Promise<number>;

  getDocument(collection: string, uid: string, queries?: Query[], forUpdate?: boolean): Promise<Document>;
...
export abstract class DatabaseAdapter implements IDatabaseAdapter {
  ...
  before(event: string, name: string = '', callback: Function | null = null): this {
  ...
  protected trigger<T extends any>(event: string, query: T): T {
    ...
    for (const callback of Object.values(this.transformations[Database.EVENT_ALL] || {})) {
