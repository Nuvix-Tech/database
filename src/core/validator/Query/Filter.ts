...
export class Filter extends Base {
  ...
  constructor(attributes: Document[] = [], maxValuesCount: number = 100, minAllowedDate: Date = new Date('0000-01-01'), maxAllowedDate: Date = new Date('9999-12-31')) {
    super();
  ...
  protected isEmpty(values: any[]): boolean {
    return values.length === 0 || (Array.isArray(values[0]) && values[0].length === 0);
  }
    ...
    switch (method) {
      ...
      case Query.TYPE_CONTAINS:
        if (this.isEmpty(value.getValues())) {
          this.message = method.charAt(0).toUpperCase() + method.slice(1) + ' queries require at least one value.';
          return false;
        }
      ...
      case Query.TYPE_ENDS_WITH:
        if (value.getValues().length !== 1) {
          this.message = method.charAt(0).toUpperCase() + method.slice(1) + ' queries require exactly one value.';
          return false;
        }
        return this.isValidAttributeAndValues(attribute, value.getValues(), method);
      ...
      case Query.TYPE_BETWEEN:
        if (value.getValues().length !== 2) {
          this.message = method.charAt(0).toUpperCase() + method.slice(1) + ' queries require exactly two values.';
          return false;
        }
        return this.isValidAttributeAndValues(attribute, value.getValues(), method);
      ...
      case Query.TYPE_AND:
        ...
        if (value.getValues().length !== filters.length) {
          this.message = method.charAt(0).toUpperCase() + method.slice(1) + ' queries can only contain filter queries';
          return false;
        }
        if (filters.length < 2) {
          this.message = method.charAt(0).toUpperCase() + method.slice(1) + ' queries require at least two queries';
          return false;
        }
        return true;
