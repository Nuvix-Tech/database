...
export class Structure extends Validator {
  ...
  protected static formats: Record<string, { callback: (params: any) => Validator, type: string }> = {};
 ...
 */
  public static getFormats(): Record<string, { callback: (params: any) => Validator, type: string }> {
    return this.formats;
  }
   ...
   */
  public static addFormat(name: string, callback: (params: any) => Validator, type: string): void {
    this.formats[name] = {
      callback: callback,
      type: type,
    };
   ...
   */
  public static getFormat(name: string, type: string): { callback: (params: any) => Validator, type: string } {
    ...
    for (const key in structure) {
      ...
      switch (type) {
        ...
        case Database.VAR_FLOAT:
          ...
          const floatMin = attribute['signed'] ? -Database.DOUBLE_MAX : 0;
          validators.push(new RangeValidator(floatMin, Database.DOUBLE_MAX, 'float'));
          break;
        ...
        case Database.VAR_DATETIME:
          validators.push(new DatetimeValidator(this.minAllowedDate, this.maxAllowedDate));
          break;
