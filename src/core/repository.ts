...
export class Repository<Entity extends { [key: string]: any }> {
   ...
   */
  create(entityLikeArray: DeepPartial<Entity & BaseEntity>[]): Array<Entity & BaseEntity>;
  ...
  create(
    ...
    if (Array.isArray(plainEntityLikeOrPlainEntityLikes)) {
      return plainEntityLikeOrPlainEntityLikes.map((entityLike) =>
        this.create(entityLike) as any
      );
    }
  ...
  async save(entity: Entity | Entity[]): Promise<void> {
    ...
    for (const e of entities) {
      let doc = await this.database.createDocument(this.metadata.getId(), new Document(e));
   ...
   */
  async find(options?: { where?: Partial<Entity & BaseEntity> }): Promise<Entity[]> {
    if (!this.database) {
      throw new Error("Database connection is not initialized.");
    }

    const queries: Query[] = Object.entries(options?.where ?? {})
      .map(([key, value]) => Query.equal(key, [value]))

    return await this.database.find(this.metadata.getId(), queries) as any;
  }

  /**
   * Finds a single entity by ID.
   */
  async findOne(where?: Partial<Entity & BaseEntity>): Promise<Entity | null> {
    if (!this.database) {
      throw new Error("Database connection is not initialized.");
    }

    const queries: Query[] = Object.entries(where ?? {})
      .map(([key, value]) => Query.equal(key, [value]))
  ...
  private getMetaData(target: Entity): Document {
    ...
    if (!keys.includes("entity") || !keys.includes("columns")) {
      throw new DatabaseError(`Entity metadata is not valid for target: ${target.constructor.name}`);
    }
    ...
    return new Document({
      $id: this.database?.getAdapter().filter(_meta.name || target.constructor.name) as string,
