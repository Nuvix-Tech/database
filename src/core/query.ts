...
export class Query {
     ...
     */
    constructor(method: QueryType, attribute: string = "", values: QueryValues = []) {
        if (
            attribute === "" &&
        ...
        if ((method === QueryType.IsNull || method === QueryType.IsNotNull) && values.length > 0) {
            throw new QueryException(`Query method "${method}" should not have values.`);
        }
     ...
     */
    public getValue(defaultValue: ScalarValue | Query | null = null): ScalarValue | Query | null {
    ...
    public static parseQuery(rawQueryObject: RawQueryObject): Query {
        const { method: rawMethod, attribute: rawAttribute, values: rawValues } = rawQueryObject;

        if (typeof rawMethod !== "string" || !Query.isMethod(rawMethod)) {
            throw new QueryException(`Invalid query method: "${rawMethod}". Must be a valid QueryType string.`);
        }
        const method: QueryType = rawMethod;

        let attribute: string = rawAttribute ?? "";
        if (typeof attribute !== "string") {
            throw new QueryException(`Invalid query attribute: "${rawAttribute}". Must be a string or undefined.`);
        }

        let values: QueryValues = [];
        if (rawValues !== undefined && rawValues !== null) {
            if (!Array.isArray(rawValues)) {
                throw new QueryException(`Invalid query values for method "${method}". Must be an array.`);
            }

            if (Query.LOGICAL_TYPES.includes(method as typeof Query.LOGICAL_TYPES[number])) {
                values = rawValues.map((val, index) => {
                    if (typeof val !== 'object' || val === null) {
                        throw new QueryException(`Invalid nested query at index ${index} for method "${method}". Expected an object.`);
                    }
                ...
                values = rawValues.map((val, index) => {
                    if (typeof val !== 'string' && typeof val !== 'number' && typeof val !== 'boolean' && val !== null) {
                        Logger.warn(`Unexpected value type for method "${method}" at index ${index}: ${typeof val}. Expected scalar.`);
                    }
    ...
    public static findCursor(queries: Query[]): Query | undefined {
        return queries.find((query) =>
            [QueryType.CursorAfter, QueryType.CursorBefore].includes(query.getMethod())
        );
    }
    ...
    public toObject(): { method: QueryType; attribute?: string; values?: any; } {
        const object: { method: QueryType; attribute?: string; values?: any; } = { method: this.method };
        ...
        } else if (this.values.length > 0 || (this.method !== QueryType.IsNull && this.method !== QueryType.IsNotNull)) {
    ...
    public toString(): string {
        try {
            ...
            const errorMessage = e instanceof Error ? e.message : String(e);
            throw new QueryException(`Failed to serialize query to JSON: ${errorMessage}`);
        }
    }
     ...
     * @returns {Query}
     */
    public static lessThanEqual(attribute: string, value: number | string): Query {
     ...
     * @returns {Query}
     */
    public static greaterThanEqual(attribute: string, value: number | string): Query {
     ...
     * @returns {Query}
     */
    public static between(attribute: string, start: ScalarValue, end: ScalarValue): Query {
    ...
    public static getByType(queries: Query[], types: QueryType[]): Query[] {
        if (types.some(type => !Query.isMethod(type))) {
            throw new QueryException("Invalid QueryType provided in types array for getByType.");
        }
        return queries
    ...
    public isNested(): boolean {
        return Query.LOGICAL_TYPES.includes(this.method as typeof Query.LOGICAL_TYPES[number]);
    }
