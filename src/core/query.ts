...
export class Query {
  ...
  public clone(): Query {
    const clonedQuery = new Query(this.method, this.attribute, [...this.values]);
    return clonedQuery;
  }
  ...
  public static parseQuery(query: any): Query {
    ...
    if (typeof method !== 'string') {
      throw new DatabaseException('Invalid query method. Must be a string.');
    }
    ...
    if (typeof attribute !== 'string') {
      throw new DatabaseException('Invalid query attribute. Must be a string.');
    }

    if (!Array.isArray(values)) {
      throw new DatabaseException('Invalid query values. Must be an array.');
    }
  ...
  public static getByType(queries: Query[], types: string[]): Query[] {
    return queries.filter(query => types.includes(query.getMethod())).map(query => query.clone());
  }
    ...
    for (const query of queries) {
      ...
      switch (method) {
        ...
        case Query.TYPE_ORDER_DESC:
          ...
          orderTypes.push(method === Query.TYPE_ORDER_ASC ? 'ASC' : 'DESC');
          break;
