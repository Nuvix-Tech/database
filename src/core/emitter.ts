...
export interface IEmitter<EventsMap extends EmitterEventMap> {
    on<K extends keyof EventsMap>(eventName: K, name: string, listener: (...args: EventsMap[K]) => void): this;
    ...
    silent<T>(callback: () => Promise<T>, listeners?: string[] | null): Promise<T>;
}
...
type BaseEventEmitterFiltered = {
    [P in Exclude<
        keyof EventEmitter,
        'on' | 'addListener' | 'removeListener' | 'off' | 'once' | 'prependListener' | 'prependOnceListener' | 'removeAllListeners'
    >]: EventEmitter[P];
};
...
export class Emitter<EventsMap extends EmitterEventMap = EmitterEventMap>
    ...
    private _namedListeners = new Map<keyof EventsMap, Map<string, (...args: any[]) => void>>();
    private _listenerSilenceStatus: Map<string, boolean> | null = new Map<string, boolean>();

    constructor() {
        super();
        (this as unknown as EventEmitter).addListener('error', (err: Error) => {
            if (!this._namedListeners.get('error')?.size) {
                console.error(`[Emitter Internal] Unhandled listener error for event: ${err.message}`, err);
            }
        });
    }
     ...
     */
    public on<K extends keyof EventsMap>(eventName: K, name: string, listener: (...args: EventsMap[K]) => void): this {
        ...
        if (listenersForEvent.has(name)) {
            throw new Error(`Listener with name "${name}" already exists for event "${String(eventName)}".`);
        }
    ...
    public off<K extends keyof EventsMap>(eventName: K, name: string): this {
        ...
        if (!listenersForEvent || !listenersForEvent.has(name)) {
            throw new Error(`Listener with name "${name}" does not exist for event "${String(eventName)}".`);
        }
     ...
     */
    public trigger<K extends keyof EventsMap>(eventName: K, ...args: EventsMap[K]): void {
     ...
     */
    private _handleListenerError(error: Error, eventName: keyof EventsMap | typeof EventsEnum.All, listenerName: string): void {
        Logger.error(`[Emitter Error] Listener "${listenerName}" for event "${String(eventName)}" failed:`, error);
